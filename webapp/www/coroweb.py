    ...
    async def __call__(self, request):
        ...
        if kw is None:
            ...
            if not self._has_var_kw_arg and self._named_kw_args:
                ...
                kw = copy
            # check named arg:
            for k, v in request.match_inifo.items():
                if k in kw:
                    logging.warning(
...
def add_stacit(app):
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'stacit')
    app.router.add_stacit('/stacit/', path)
...
def add_routes(app, module_name):
    n = module_name.rfind('.')
    if n == (-1):
        mod = __import__(module_name, globals(), local())
    else:
